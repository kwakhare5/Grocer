...
@pytest.mark.asyncio
async def test_semantic_confirmation_triggers_confirmation() -> None:
    ...
    async def interpret_confirmation(**kwargs):  # type: ignore[no-untyped-def]
        del kwargs
    ...
    channel._conversation_interpreter.interpret = interpret_confirmation

    # The interpreter, not a prefix rule, marks this reply as confirmation.
...
@pytest.mark.asyncio
async def test_basket_change_never_confirms_checkout() -> None:
    from backend.intent.conversation import ConversationAction, ConversationCommand
    from backend.intent.session import default_session_store
    ...
    store = default_session_store
    orchestrator = GrocerOrchestrator(commerce_adapter=adapter, session_store=store)
    customer_id = channel.map_sender_to_customer_id("+919876543212")
    session_id = channel.get_or_create_session_id(customer_id)

    await orchestrator.handle_turn(session_id, customer_id, "get 1L milk", address_id="addr-1")
    session = store.get(session_id)
    assert session.conversation_state == ConversationState.AWAITING_CONFIRMATION

    async def interpret_change(**kwargs):  # type: ignore[no-untyped-def]
        del kwargs
        return ConversationCommand(action=ConversationAction.BASKET_REQUEST, confidence=1)

    channel._conversation_interpreter.interpret = interpret_change
    response = await channel.dispatch(
        NormalizedIncomingMessage(
            message_id="msg-change-not-confirm",
            channel=ChannelType.WHATSAPP,
            sender_id="+919876543212",
            text="ok add eggs",
            session_id=session_id,
        ),
        orchestrator,
    ...
    assert response.conversation_state != ConversationState.ORDERED
