...
### [GROCER — Whitelisted OAuth Gateway, Token Vault PostgreSQL Durability & History Consolidation] 2026-09-19

- **Problem / tension**: Swiggy's OAuth gateway strictly whitelists `https://grocerr.vercel.app/` as the only permitted `redirect_uri` for client ID `swiggy-mcp`. Live WhatsApp expiration prompts previously directed users to direct Render URLs (`https://grocer-backend-qwk4.onrender.com/connect?...`), triggering redirect mismatches on Swiggy. Furthermore, when completing the OAuth exchange callback, `POST /auth/swiggy/callback` invoked synchronous `default_token_vault.store_token()`, triggering a fatal `RuntimeError` on PostgreSQL-backed token stores where `store_token_durable` is mandatory. Additionally, the repository history had accumulated fragmented commits following cleanroom recovery.
- **Change / decision**: Re-routed all customer connect URLs to `https://grocerr.vercel.app/?phone=<phone>` across `backend/config.py` (`CONNECT_BASE_URL`), `backend/agent/engine.py`, and `app/page.tsx` with automated 10-digit phone prefill. Added HTTP 307 auto-redirect on `GET /connect` to seamlessly forward any legacy links to the whitelisted domain. Swapped synchronous token vault calls in `backend/api/oauth.py` to `await default_token_vault.store_token_durable()`. Squashed the cleanroom era down to 9 clean milestone commits and synchronized both `main` and `dev-live-test` with remote `origin`.
- **Proof**: Verified live Vercel proxy (`POST https://grocerr.vercel.app/api/auth/swiggy/login`) successfully forwards to Render and returns valid Swiggy authorization URL with `redirect_uri=https://grocerr.vercel.app/`. Tested `GET https://grocer-backend-qwk4.onrender.com/connect?customer_id=...` and verified instant HTTP 307 redirect to `https://grocerr.vercel.app/?customer_id=...`. Backend pytest suite passed 258/258 tests hermetically in 2.66s. Frontend `npm run lint` and Next.js 16 Turbopack build passed cleanly with 0 errors. GitHub Actions `Quality` workflow passed 100% green on commit `d463d47`.
- **Still broken / unproven**: The active WhatsApp customer session requires completing the 1-time Swiggy authentication flow via `https://grocerr.vercel.app/` (since the previous static token expired) before Swiggy MCP tools can place orders on physical WhatsApp.
- **Metric context**: 258 tests passing in 2.66s; 0 lint or build errors; GitHub Actions CI 100% green; 100% whitelisted OAuth compliance.
- **Trial-ready flow**: User receives reconnect prompt on WhatsApp -> taps link redirecting to `https://grocerr.vercel.app/?phone=91...` -> enters Swiggy OTP -> access token stored durably in PostgreSQL -> user messages *"i need bread and eggs"* on WhatsApp -> agent adds items to cart and delivers UPI checkout link.
- **Engineering references**: `backend/api/oauth.py`, `backend/agent/engine.py`, `backend/config.py`, `app/page.tsx`, `app/api/auth/swiggy/login/route.ts`.
