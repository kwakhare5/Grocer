...
async def _search_and_pick(
    ...
    is_liquid_cat = item.category in {"beverages", "dairy"} or any(
        term in item.name.lower() for term in ("coke", "pepsi", "soda", "juice", "drink", "milk", "water")
    )
    if catalog_dependent and individual_variants and not is_liquid_cat:
    ...
    pref_format = (item.unit or "").lower()
    if pref_format not in {"can", "tin", "bottle", "pouch", "sachet", "box", "tetrapak", "strip", "bar"}:
        pref_format = (item.pack_size_preference or "").lower()

    def rank(candidate: tuple[CommerceProductItem, Any, int, ResolvedMeaning]) -> tuple[bool, bool, bool, float, float]:
        _product, variant, pack_count, _meaning = candidate
        candidate_pack = normalize_pack_quantity(variant.pack_size)
        candidate_text = f"{_product.name} {variant.name} {variant.pack_size}".lower()

        misses_preference = preferred_pack is not None and candidate_pack != preferred_pack

        misses_format = False
        if pref_format in {"can", "tin", "bottle", "pouch", "sachet", "box", "tetrapak", "strip", "bar"}:
            fmt_synonyms = {
                "can": ["can", "tin"],
                "tin": ["tin", "can"],
                "bottle": ["bottle", "pet"],
                "pouch": ["pouch", "packet"],
                "tetrapak": ["tetrapak", "tetrapack", "tetra"],
            }
            keywords = fmt_synonyms.get(pref_format, [pref_format])
            misses_format = not any(kw in candidate_text for kw in keywords)

        is_unwanted_multipack = False
        if item.quantity == 1 and any(
            mp in candidate_text
            for mp in ["*8", "*6", "*4", "*12", "pack of 8", "pack of 6", "pack of 4", "multipack", "multi-pack", " mp "]
        ):
            is_unwanted_multipack = True

        return misses_preference, misses_format, is_unwanted_multipack, variant.price * pack_count, variant.price
...
def _is_fresh_request(message: str) -> bool:
    """Heuristic: treat message as explicit request to reset session vs a refinement."""
    lower = message.lower().strip()
    if _is_incremental_add(message) or _detect_swap_request(message) or _detect_removal_request(message):
        return False
    reset_keywords = {
        "start over", "start fresh", "new order", "new basket",
        "clear cart", "clear basket", "empty cart", "empty basket",
        "reset", "cancel order", "delete everything",
    }
    return any(kw in lower for kw in reset_keywords)
...
def _merge_contracts(
    ...
    if new.items:
        existing_by_name = {it.name.lower(): it for it in merged.items}
        for it in new.items:
            key = it.name.lower()
            if key in existing_by_name:
                existing_item = existing_by_name[key]
                existing_item.quantity = it.quantity
                if it.unit and it.unit != "units":
                    existing_item.unit = it.unit
                if it.brand_preference:
                    existing_item.brand_preference = it.brand_preference
                if it.pack_size_preference:
                    existing_item.pack_size_preference = it.pack_size_preference
            else:
...
def has_item_modifications(message: str, parsed_contract: Optional[IntentContract] = None) -> bool:
    """Detect if incoming message contains item additions, removals, swaps, or quantities."""
    if _detect_swap_request(message) or _detect_removal_request(message):
        return True
    if parsed_contract and parsed_contract.items:
        return True
    return False
