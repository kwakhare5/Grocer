...
### [GROCER — Phase 0–6: Boundary Cleanup, Intent Contract, Extraction, Policy, Verifier, Recovery & Agent Orchestration] 2026-09-06
...
- **Phase 6 Test Suite (`backend/tests/test_orchestrator.py`)**: 20 tests covering happy path checkout, session persistence, multi-item basket, budget constraints, clarification candidate resolution, state machine invariants, unconfirmed checkout rejection, and full REST API lifecycle. Full suite: **122/122 passing** (100% green). Build: **✓ clean**.
- **Phase 5 Recovery Engine (`backend/intent/recovery.py`)**: Deterministic `RecoveryEngine` implementing Spec §10 closed-loop recovery and candidate ranking. Enums: `RecoveryState` (RECOVERED, NEEDS_USER_DECISION, BLOCKED, FAILED), `FailureClass` (9 categories: item unavailable, brand unavailable, pack size changed, budget drift, stale cart, transient error, partial success, min-order failure, unknown). Models: `RecoveryAction`, `RecoveryCandidate`, `RecoveryOutcome`. Features: candidate generation from available commerce products, PolicyEngine hard constraint filtering, 7-dimension scoring (brand affinity, pack size similarity, price impact). Handlers for missing items, brand locks/substitutions, pack size multiples (2x 500ml for 1L), budget drift (downsize/swap), minimum-order basket repair, safe transient retry, and stale cart escalation. Bounded recovery strictly aborts with FAILED when `attempt_number > max_attempts`. Unified closed-loop execution `execute_recovery()` coordinates `CommercePort` cart mutation and `IntentVerifier` re-verification.
- **Phase 5 Test Suite (`backend/tests/test_recovery_engine.py`)**: 14 tests — single clear substitute auto-recovery, close candidates ambiguity escalation, no category candidate blocked, dietary violation candidate filtered, ordered alternative brand auto-recovery, budget drift item swap, budget drift impossible escalation, stale cart escalation, max attempts exceeded, clean cart pass, pack size multiples auto-recovery, min-order threshold escalation, transient error safe retry, and full async closed-loop recovery with `MockCommerceAdapter` and `IntentVerifier`. Full suite: **102/102 passing** (100% green). Build: **✓ clean**.
- **Phase 4 Intent Verifier (`backend/intent/verifier.py`)**: Deterministic `IntentVerifier` implementing Spec §9 cart-vs-intent comparison. Zero LLM dependency. Eight check methods: budget, dietary, missing items, exact quantity, brand (hard lock + soft deviation), pack size, stale cart, checkout authorization. `VerificationStatus` (PASS/FAIL), `ViolationCode` (8 codes), `ConstraintViolation`, `PreferenceDeviation`, `VerificationResult` models. Hard violations → FAIL always. Soft deviations accumulate without forcing FAIL. `verify()` for post-mutation checks, `verify_checkout()` always fails without `explicit_confirmation=True` (Spec §8.3 double-gate).
- **Phase 4 Test Suite (`backend/tests/test_intent_verifier.py`)**: 14 tests — exact pass, budget hard breach, budget within deviation, missing item, exact quantity, hard brand lock, soft brand deviation, strict pack size, stale cart (flag + non-serviceable), dietary violation (non-veg in veg contract), checkout without/with confirmation, recovery re-verify. Full suite: **88/88 passing**. Build: **✓ clean**.
- **Phase 3 Policy Engine (`backend/intent/policy.py`)**: Deterministic `PolicyEngine` implementing Spec §6 three-level autonomy model. Evaluates `ActionProposal` against `IntentContract` → returns `PolicyDecision` with `AutonomyLevel` (AUTO_EXECUTE / ASK_USER / REQUIRE_CONFIRMATION / BLOCKED). Checkout always requires confirmation. Dietary violations and brand locks → BLOCKED. Budget breaches evaluated against deviation allowance.
- **Phase 3 Preference Store (`backend/intent/preferences.py`)**: Per-customer durable memory with `StoredPreference` model and evidence tracking. Rejects non-storable data per Spec §7.2 (old prices, one-off observations). Preferences must be established (evidence ≥ 2) before influencing contracts. Converts to domain `BrandPreference`/`SoftPreference` models for `apply_memory()`.
- **Phase 3 Test Suite (`backend/tests/test_policy_engine.py`)**: 10 tests covering checkout confirmation, safe/ambiguous/blocked substitutions, dietary violation, budget breach (zero vs deviation), brand lock, preference store CRUD, stale rejection, and memory integration. Full suite: **74/74 passing**. Build: **✓ clean**.
- **Phase 2 Intent Parser**: `RuleBasedExtractor` + `DeterministicValidator`, contradiction/ambiguity detection, multi-word brand extraction. 10 tests.
- **Phase 1 Intent Contract**: Domain models, precedence engine, authorization scope, session store. 8 tests.
