...
3. **`SwiggyTokenVault`** is a development cache only. A submission or production deployment must use durable encrypted token storage; local `/tmp` files do not survive Render restarts reliably.
...
5. **Review Checkout Guard:** `CHECKOUT_MODE=review` keeps the real cart and verification journey available while truthfully stopping before a chargeable checkout. `CHECKOUT_MODE=live` is an explicit deployment decision and requires durable state plus real integration verification.
