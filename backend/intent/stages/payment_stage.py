...
from backend.intent.formatters import format_payment_choice as _msg_payment_choice
...
def match_payment_choice(
    choice: str,
    options: list[PaymentOption],
) -> Optional[PaymentOption]:
    """Match user payment selection by index, method, or label keyword."""
    ...
    for opt in options:
        if (
            (opt.id and opt.id.lower() == cleaned)
            or opt.method.lower() == cleaned
            or opt.label.lower() == cleaned
        ):
            return opt
    for opt in options:
        if cleaned in opt.label.lower() or cleaned in opt.method.lower():
            return opt
    return None
