...
### [GROCER — Cloud Proxy Healing, Zero-Leak Token Sync & Live WhatsApp Shopping] 2026-09-19

- **Problem / tension**: WhatsApp repeatedly replied "Your Swiggy login has expired" because Render's cloud container booted fresh with an empty token vault, while Vercel's proxy (`app/api/_lib/backend.ts`) threw `502 Bad Gateway` ("Grocer is temporarily unavailable") due to missing `BACKEND_INTERNAL_URL` configuration, permanently blocking browser OAuth reconnect callbacks from reaching the Render backend. Furthermore, GitGuardian flagged an HMAC hex test identifier in test fixtures as high entropy.
- **Change / decision**: Fixed `app/api/_lib/backend.ts` to automatically fall back to production `https://grocer-backend-qwk4.onrender.com` (and `INTENT_BACKEND_URL`), eliminating the 502 Bad Gateway. Added an internal authenticated `POST /api/auth/token/sync` endpoint in `backend/api/oauth.py` protected by `WHATSAPP_APP_SECRET` to securely seed active provider tokens into Render's vault and disk storage (`.vault_tokens.json`) without exposing secrets in git or dashboard delays. Scrubbed high-entropy test fixtures, added reentrant locks (`RLock`) to prevent file I/O deadlocks, and updated both local `.env` files with the active Sept 23 token.
- **Proof**: 262/262 backend pytest tests passing green in 2.58s. Frontend `npm run lint` clean (0 errors); Next.js 16 Turbopack production build passing in 2.1s. Deployed to `origin/main` (commit `a6085e0`) and `origin/dev-live-test` (commit `97835bc`). Verified live: Vercel `/api/auth/swiggy/login` returned `200 OK`. Token synced to Render live container with `HTTP 200 OK`. Simulated live WhatsApp webhook with message "i need bread and eggs": Render processed with status 200, autonomously resolved Pune Kingsbury dark store, selected NOICE High Protein Eggs, identified out-of-stock bread, and updated live Swiggy Instamart cart ID `aa07e9e7` to ₹125.
- **Still broken / unproven**: Update `SWIGGY_AUTH_TOKEN` in Render dashboard environment variables for cold-start boot persistence across future container rebuilds.
- **Metric context**: 262 tests passing (100% green); 0 lint errors; 2.1s Turbopack build; live cloud webhook latency <6s; 1 active Swiggy cart generated live on Cloud Render.
- **Trial-ready flow**: User texts "i need bread and eggs" on WhatsApp -> agent picks Pune Kingsbury address, searches inventory, adds available eggs to Swiggy cart, flags unavailable bread, presents ₹125 total summary, and awaits checkout confirmation.
- **Engineering references**: `app/api/_lib/backend.ts`, `backend/api/oauth.py`, `backend/integrations/commerce/token_vault.py`, `backend/tests/test_swiggy_oauth.py`.
