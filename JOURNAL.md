...
### [GROCER — Whitelisted OAuth Gateway, Token Vault Reliability, Pune Address Routing & URL Privacy] 2026-09-19

- **Problem / tension**: Reconnect links on WhatsApp previously included the customer's phone number as a query parameter (`?phone=918237803170`), exposing private data in chat and in browser URL bars. On the cloud server (Render), cold starts or restarts cleared process memory, causing false-positive "Your Swiggy login has expired" loops even though the underlying token was valid until Sept 22. Furthermore, Swiggy's address list unranked array caused the bot to blindly pick Nashik (`addresses[0]`) instead of the user's primary residence in Pune (Kingsbury), and Gemini tool schemas marked `sku_id` as optional in `update_cart`, risking `MISSING_SKU` rejections from Swiggy Instamart.
- **Change / decision**: Stripped all query parameters (`?phone=`, `?customer_id=`) from reconnect URLs across `backend/agent/engine.py`, `backend/api/oauth.py`, and `app/page.tsx`, making all public links clean (`https://grocerr.vercel.app/`). Added fallback in `backend/integrations/commerce/token_vault.py` to `settings.SWIGGY_AUTH_TOKEN` when in-memory cache is empty for the owner customer ID, eliminating cold-start expiration loops. Upgraded address resolution in `engine.py` to prioritize Kingsbury/Pune as the smart starting default, and introduced `select_delivery_address` tool allowing full conversational destination switching (e.g. "deliver to Nashik"). Made `sku_id` strictly mandatory in `update_cart` Gemini tool declaration. Added `SWIGGY_AUTH_TOKEN` and `SWIGGY_CUSTOMER_ID` to `render.yaml`.
- **Proof**: 261/261 backend pytest tests passing green in 2.96s hermetically. Added 3 new tests: `test_smart_pune_default_address_prioritization`, `test_select_delivery_address_tool`, and `test_token_vault_fallback_to_configured_swiggy_auth_token`. Frontend `npm run lint` clean (0 errors, 0 warnings); Next.js 16 Turbopack production build compiled in 2.4s.
- **Still broken / unproven**: Setting `SWIGGY_AUTH_TOKEN` on Render deployment dashboard to complete live physical WhatsApp order placement.
- **Metric context**: 261 tests passing (100% green); 0 lint errors; 2.4s Turbopack build; 100% private clean URLs; 3 saved addresses dynamically routable.
- **Trial-ready flow**: User texts "i need bread and eggs" on WhatsApp -> agent selects Pune Kingsbury dark store, finds in-stock items with spin_id and sku_id, syncs Swiggy cart, shows order summary, and generates UPI checkout link upon approval. User can say "deliver to Nashik" to switch delivery cities on demand.
- **Engineering references**: `backend/agent/engine.py`, `backend/agent/tools.py`, `backend/integrations/commerce/token_vault.py`, `backend/api/oauth.py`, `app/page.tsx`, `render.yaml`.
